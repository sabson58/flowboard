...
function CardDetailModal({ card, columnId, boardId, onClose, onSave, onDelete }) {
  ...
  async function addComment() {
    ...
    await addDoc(
      collection(db, 'boards', boardId, 'columns', columnId, 'cards', card.id, 'comments'),
      { text: comment.trim(), userId: user.uid, userName: user.displayName || user.email, createdAt: Date.now() }
    )
    setComment('')
  }

  async function deleteComment(commentId) {
    await deleteDoc(doc(db, 'boards', boardId, 'columns', columnId, 'cards', card.id, 'comments', commentId))
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '3rem 1rem', overflowY: 'auto' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, width: '100%', maxWidth: 680, boxShadow: '0 32px 100px rgba(0,0,0,0.7)', animation: 'slideUp 0.25s ease', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: '0.75rem' }}>
            {editingTitle ? (
              <input value={title} onChange={e => setTitle(e.target.value)} onBlur={() => { setEditingTitle(false); handleSave() }} onKeyDown={e => { if (e.key === 'Enter') { setEditingTitle(false); handleSave() } if (e.key === 'Escape') setEditingTitle(false) }} autoFocus style={{ flex: 1, fontSize: 20, fontWeight: 700, background: 'var(--surface2)', border: '1px solid var(--accent)', borderRadius: 8, padding: '6px 10px', color: 'var(--text)', outline: 'none', fontFamily: 'Inter, sans-serif', letterSpacing: '-0.02em' }} />
            ) : (
              <h2 onClick={() => setEditingTitle(true)} style={{ flex: 1, fontSize: 20, fontWeight: 700, letterSpacing: '-0.02em', cursor: 'text', color: 'var(--text)', lineHeight: 1.3 }} title="Click to edit">{title}</h2>
            )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px' }}>
          <div style={{ padding: '1.5rem', borderRight: '1px solid var(--border)' }}>
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Add a description..." rows={4} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px', color: 'var(--text)', fontSize: 14, resize: 'vertical', outline: 'none', fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }} onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => { e.target.style.borderColor = 'var(--border)'; handleSave() }} />
            </div>
            <div>
              ...
              <div style={{ marginBottom: 12 }}>
                {comments.map(c => (
                  <div key={c.id} style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'white', flexShrink: 0 }}>{c.userName?.[0]?.toUpperCase() || '?'}</div>
                    <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'white', flexShrink: 0, marginTop: 2 }}>{user?.displayName?.[0]?.toUpperCase() || '?'}</div>
                <div style={{ flex: 1 }}>
                  <textarea value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); addComment() } }} placeholder="Write a comment... (Enter to post)" rows={2} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 12px', color: 'var(--text)', fontSize: 13, resize: 'none', outline: 'none', fontFamily: 'Inter, sans-serif' }} onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
          ...
          <div style={{ padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 8 }}>
                {labels.map((l, i) => <span key={i} onClick={() => setLabels(labels.filter((_, j) => j !== i))} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 5, cursor: 'pointer', background: 'var(--accent-dim)', color: 'var(--accent2)', border: '1px solid rgba(124,106,255,0.2)' }} title="Click to remove">{l} ×</span>)}
              </div>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              {card.assignedTo ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'white' }}>{card.assignedName?.[0]?.toUpperCase()}</div>
                  <span style={{ fontSize: 12, color: 'var(--text)', flex: 1 }}>{card.assignedName}</span>
                  <button onClick={async () => { await updateDoc(doc(db, 'boards', boardId, 'columns', columnId, 'cards', card.id), { assignedTo: null, assignedName: null }) }} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 14 }}>×</button>
                </div>
              ) : <p style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 8 }}>Not assigned</p>}
              <button onClick={async () => { await updateDoc(doc(db, 'boards', boardId, 'columns', columnId, 'cards', card.id), { assignedTo: user.uid, assignedName: user.displayName || user.email }); await logActivity(boardId, user, `self-assigned a card`) }} style={{ width: '100%', padding: '7px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.color = 'var(--accent)'; e.currentTarget.style.background = 'var(--accent-dim)' }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text2)'; e.currentTarget.style.background = 'var(--surface2)' }}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: 'white' }}>{user?.displayName?.[0]?.toUpperCase() || '?'}</div>
                Assign to me
              </button>
            </div>
            {columnId !== 'done' && (
              <button onClick={async () => { await deleteDoc(doc(db, 'boards', boardId, 'columns', columnId, 'cards', card.id)); await addDoc(collection(db, 'boards', boardId, 'columns', 'done', 'cards'), { ...card, order: Date.now(), createdAt: Date.now() }); onClose(); confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 }, colors: ['#7c6aff', '#4dff91', '#ffd44d', '#ff4d4d'] }) }} style={{ width: '100%', padding: '8px', background: 'rgba(77,255,145,0.1)', border: '1px solid rgba(77,255,145,0.25)', borderRadius: 8, color: 'var(--green)', fontSize: 12, fontWeight: 600, cursor: 'pointer', marginBottom: 8, transition: 'all 0.15s', fontFamily: 'Inter, sans-serif' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(77,255,145,0.2)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(77,255,145,0.1)'}>🎉 Mark as Done</button>
            )}
            <button onClick={() => { if (confirm('Delete this card?')) { onDelete(card.id, columnId); onClose() } }} style={{ width: '100%', padding: '8px', background: 'var(--red-dim)', border: '1px solid rgba(255,77,77,0.2)', borderRadius: 8, color: 'var(--red)', fontSize: 12, fontWeight: 600, cursor: 'pointer', marginTop: '1rem' }}>🗑 Delete card</button>
          </div>
        </div>
      </div>
    </div>
...
const sideLabel = { fontSize: 11, fontWeight: 700, color: 'var(--text2)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      ...
      >
        ...
        {columnId === 'done' && (
          <div style={{ position: 'absolute', top: 8, right: 8, width: 18, height: 18, borderRadius: '50%', background: 'rgba(77,255,145,0.15)', border: '1px solid rgba(77,255,145,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--green)' }}>✓</div>
        )}
        <div onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); onOpen(card, columnId) }} style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', lineHeight: 1.5, marginBottom: 10, cursor: 'pointer', transition: 'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--accent2)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--text)'}>{card.title}</div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {card.assignedTo && <div title={card.assignedName} style={{ width: 22, height: 22, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'white', border: '2px solid var(--surface2)', flexShrink: 0 }}>{card.assignedName?.[0]?.toUpperCase() || '?'}</div>}
...
function KanbanColumn({ column, cards, onAddCard, onEditCard, onDeleteCard, onOpenCard, onRename, onDeleteColumn }) {
  const { attributes, listeners, setNodeRef: setSortableRef, transform, transition, isDragging } = useSortable({ id: column.id, data: { type: 'column', column } })
  const { setNodeRef: setDroppableRef, isOver } = useDroppable({ id: column.id, data: { columnId: column.id } })
  ...
  return (
    <div ref={setSortableRef} style={{ ...style, width: 300, flexShrink: 0, background: 'var(--surface)', border: `1px solid ${isOver ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 16, display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 140px)', transition: 'border-color 0.2s' }}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, cursor: 'grab' }} {...attributes} {...listeners}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: column.color, flexShrink: 0, boxShadow: `0 0 8px ${column.color}` }}/>
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.01em', cursor: 'text' }} onPointerDown={e => e.stopPropagation()} onDoubleClick={() => onRename(column.id, column.title)} title="Double-click to rename">{column.title}</span>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); setAdding(true) }} style={{ width: 26, height: 26, borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text2)', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--accent-dim)'; e.currentTarget.style.color = 'var(--accent)'; e.currentTarget.style.borderColor = 'var(--accent)' }} onMouseLeave={e => { e.currentTarget.style.background = 'var(--surface2)'; e.currentTarget.style.color = 'var(--text2)'; e.currentTarget.style.borderColor = 'var(--border)' }}>+</button>
          <button onPointerDown={e => e.stopPropagation()} onClick={e => { e.stopPropagation(); onDeleteColumn(column.id) }} style={{ width: 26, height: 26, borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text3)', cursor: 'pointer', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--red-dim)'; e.currentTarget.style.color = 'var(--red)'; e.currentTarget.style.borderColor = 'rgba(255,77,77,0.3)' }} onMouseLeave={e => { e.currentTarget.style.background = 'var(--surface2)'; e.currentTarget.style.color = 'var(--text3)'; e.currentTarget.style.borderColor = 'var(--border)' }} title="Delete column">✕</button>
        </div>
      </div>
      <div ref={setDroppableRef} style={{ padding: '12px', overflowY: 'auto', flex: 1, background: isOver ? 'rgba(124,106,255,0.05)' : 'transparent', transition: 'background 0.2s', minHeight: 80 }}>
        ...
        {cards.length === 0 && !adding && (
          <div style={{ textAlign: 'center', padding: '2rem 1rem', color: isOver ? 'var(--accent)' : 'var(--text3)', fontSize: 13, border: `2px dashed ${isOver ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 10, transition: 'all 0.2s' }}>
        ...
        {adding && (
          <div style={{ background: 'var(--surface2)', border: '1px solid var(--accent)', borderRadius: 10, padding: 10, marginTop: 4 }}>
            <textarea value={newTitle} onChange={e => setNewTitle(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleAdd() } if (e.key === 'Escape') { setAdding(false); setNewTitle('') } }} placeholder="Card title... (Enter to save)" autoFocus rows={2} style={{ width: '100%', background: 'none', border: 'none', color: 'var(--text)', fontSize: 14, resize: 'none', outline: 'none', fontFamily: 'Inter, sans-serif', marginBottom: 8 }} />
...
function InviteModal({ boardId, board, user, onClose, memberProfiles }) {
  ...
  async function handleInvite() {
    ...
    else { setStatus({ type: 'success', msg: `${email} has been added to the board!` }); await logActivity(boardId, user, `invited ${email} to the board`); setEmail('') }
    setLoading(false)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: '1.75rem', width: '100%', maxWidth: 440, boxShadow: '0 24px 80px rgba(0,0,0,0.6)', animation: 'slideUp 0.25s ease' }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: '1.25rem', lineHeight: 1.6 }}>Invite a teammate by their email. They need to have a FlowBoard account first.</p>
        <div style={{ display: 'flex', gap: 8, marginBottom: '1rem' }}>
          <input value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleInvite()} placeholder="teammate@email.com" type="email" autoFocus style={{ flex: 1, padding: '10px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 14, color: 'var(--text)', outline: 'none', fontFamily: 'Inter, sans-serif' }} onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
          <button onClick={handleInvite} disabled={!email.trim() || loading} style={{ padding: '10px 18px', background: 'var(--accent)', border: 'none', borderRadius: 8, color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: !email.trim() ? 0.5 : 1, whiteSpace: 'nowrap' }}>{loading ? 'Inviting...' : 'Invite'}</button>
        </div>
        {status && <div style={{ padding: '10px 14px', borderRadius: 8, fontSize: 13, background: status.type === 'success' ? 'rgba(77,255,145,0.1)' : 'var(--red-dim)', color: status.type === 'success' ? 'var(--green)' : 'var(--red)', border: `1px solid ${status.type === 'success' ? 'rgba(77,255,145,0.2)' : 'rgba(255,77,77,0.2)'}` }}>{status.msg}</div>}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {(board?.members || []).map((uid, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 20, fontSize: 12 }}>
                <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'white' }}>{uid === user.uid ? user.displayName?.[0]?.toUpperCase() : '?'}</div>
                <span style={{ color: 'var(--text2)' }}>{uid === user.uid ? `You (${user.displayName || user.email})` : (memberProfiles?.[uid]?.name || memberProfiles?.[uid]?.email || 'Member')}</span>
              </div>
            ))}
          </div>
        </div>
...
function AIGenerateModal({ boardId, user, onClose, onGenerate }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      onClose()
    } catch (e) { setError('Failed to generate tasks. Check your API connection and try again.') }
    setLoading(false)
  }

  const examples = ['Build a mobile e-commerce app', 'Launch a company blog', 'Redesign the company website', 'Build a user authentication system']
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: '1.75rem', width: '100%', maxWidth: 500, boxShadow: '0 24px 80px rgba(0,0,0,0.7)', animation: 'slideUp 0.25s ease' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <div><h3 style={{ fontSize: 18, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 4 }}>✨ AI Task Generator</h3><p style={{ fontSize: 13, color: 'var(--text2)' }}>Describe your project and AI will fill your board</p></div>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text2)', cursor: 'pointer', fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>×</button>
        </div>
        <div style={{ marginBottom: '1rem', marginTop: '1.25rem' }}>
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && e.metaKey) handleGenerate() }} placeholder="e.g. Build a React e-commerce store..." rows={3} autoFocus style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', color: 'var(--text)', fontSize: 14, resize: 'none', outline: 'none', fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }} onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
        </div>
        <div style={{ marginBottom: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {examples.map((ex, i) => <button key={i} onClick={() => setPrompt(ex)} style={{ padding: '5px 12px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 20, color: 'var(--text2)', fontSize: 12, cursor: 'pointer', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.color = 'var(--accent)' }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text2)' }}>{ex}</button>)}
          </div>
        </div>
        {error && <div style={{ padding: '10px 14px', borderRadius: 8, fontSize: 13, background: 'var(--red-dim)', color: 'var(--red)', border: '1px solid rgba(255,77,77,0.2)', marginBottom: '1rem' }}>{error}</div>}
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 11, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, color: 'var(--text2)', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
          <button onClick={handleGenerate} disabled={!prompt.trim() || loading} style={{ flex: 2, padding: 11, background: loading ? 'var(--surface3)' : 'var(--accent)', border: 'none', borderRadius: 10, color: 'white', fontSize: 14, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', opacity: !prompt.trim() ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            {loading ? (<><div style={{ width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }}/>Generating tasks...</>) : '✨ Generate Tasks'}
          </button>
        </div>
...
function ShortcutsModal({ onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 20, padding: '1.75rem', width: '100%', maxWidth: 400, boxShadow: '0 24px 80px rgba(0,0,0,0.6)', animation: 'slideUp 0.25s ease' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {shortcuts.map((s, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--surface2)', borderRadius: 10, border: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>{s.keys.map((k, j) => <kbd key={j} style={{ padding: '3px 10px', background: 'var(--surface3)', border: '1px solid var(--border)', borderRadius: 6, fontSize: 12, fontWeight: 700, color: 'var(--text)', fontFamily: 'Inter, sans-serif', boxShadow: '0 2px 0 var(--border)' }}>{k}</kbd>)}</div>
            </div>
          ))}
        </div>
        <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: '1.25rem', textAlign: 'center' }}>Shortcuts are disabled when typing in an input</p>
      </div>
    </div>
  )
}
...
function StatsPanel({ cards, columns, columnOrder, board, onClose }) {
  ...
  const overdue = allCards.filter(c => { if (!c.dueDate) return false; return new Date(c.dueDate) < new Date() }).length
  ...
  return (
    <div style={{ position: 'fixed', top: 56, right: 0, bottom: 0, width: 360, background: 'var(--surface)', borderLeft: '1px solid var(--border)', zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'slideInRight 0.25s ease', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            ...
            <div key={kpi.label} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px', textAlign: 'center', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--border2)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
...
function ActivityPanel({ log, onClose }) {
  return (
    <div style={{ position: 'fixed', top: 56, right: 0, bottom: 0, width: 300, background: 'var(--surface)', borderLeft: '1px solid var(--border)', zIndex: 50, display: 'flex', flexDirection: 'column', animation: 'slideInRight 0.25s ease' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
        {log.length === 0 && <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text3)', fontSize: 13 }}>No activity yet</div>}
        {log.map(entry => (
          <div key={entry.id} style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'white' }}>{entry.userName?.[0]?.toUpperCase() || '?'}</div>
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2 }}>{new Date(entry.createdAt).toLocaleDateString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
            </div>
          </div>
        ))}
      </div>
...
export default function BoardPage() {
  ...
  // ── KEY FIX: ref to block Firestore listener during drag ──
  ...
  useKeyboard([
    ...
    { key: 'Escape', action: () => { setOpenCard(null); setOpenCardColumnId(null); setShowCardModal(false); setShowActivity(false); setShowInvite(false); setShowShortcuts(false); setShowAI(false); setShowStats(false); setSearch('') } },
    ...
    async function loadProfiles() {
      ...
      for (const uid of board.members) {
        try {
          ...
          profiles[uid] = userSnap.exists() ? userSnap.data() : { name: 'Member', email: '' }
        } catch (e) { profiles[uid] = { name: 'Member', email: '' } }
      }
  ...
  useEffect(() => {
    if (!boardId) return
    const q = query(collection(db, 'boards', boardId, 'activity'), orderBy('createdAt', 'desc'))
    const unsub = onSnapshot(q, snap => { setActivityLog(snap.docs.map(d => ({ id: d.id, ...d.data() }))) })
    return unsub
  }, [boardId])

  // ── CARDS LISTENER — blocks updates while dragging ────────
  useEffect(() => {
    if (!columnsRef.current.length) return
    const unsubs = columnsRef.current.map(col => {
      const q = query(
        collection(db, 'boards', boardId, 'columns', col.id, 'cards'),
        orderBy('order', 'asc')
      )
      return onSnapshot(q, snap => {
        const seen = new Set()
        const newColCards = snap.docs
        ...
        if (isDraggingRef.current) {
          pendingCardsRef.current = { ...(pendingCardsRef.current || {}), [col.id]: newColCards }
        } else {
          setCards(prev => ({ ...prev, [col.id]: newColCards }))
        }
      })
    })
  ...
  async function addCard(columnId, title) {
    ...
    await logActivity(boardId, user, `added "${title}" to ${DEFAULT_COLUMNS.find(c => c.id === columnId)?.title}`)
  }

  function openEditCard(card, columnId) { setEditingCard(card); setEditingColumnId(columnId); setShowCardModal(true) }

  async function saveCard(data) {
    if (editingCard) await updateDoc(doc(db, 'boards', boardId, 'columns', editingColumnId, 'cards', editingCard.id), data)
  ...
  async function deleteCard(cardId, columnId) {
    try {
      ...
      for (const [colId, colCards] of Object.entries(cards)) {
        ...
        if (found) { actualColumnId = colId; cardData = found; break }
      }
      setCards(prev => ({ ...prev, [actualColumnId]: (prev[actualColumnId] || []).filter(c => c.id !== cardId) }))
  ...
  function openCardDetail(card, columnId) { setOpenCard(card); setOpenCardColumnId(columnId) }

  async function generateTasks(tasks) {
    for (const task of tasks) {
      const colCards = cards[task.column] || []
      await addDoc(collection(db, 'boards', boardId, 'columns', task.column, 'cards'), { title: task.title, priority: task.priority, labels: [], dueDate: '', order: colCards.length, createdAt: Date.now(), createdBy: user.uid })
    }
  ...
  async function addColumn() {
    ...
    const color = ['#7c6aff','#f59e0b','#10b981','#3b82f6','#f43f5e'][Math.floor(Math.random() * 5)]
    const newCol = { id, title: title.trim(), color }
    const newColumns = [...columns, newCol]
    const newOrder = [...columnOrder, id]
    setColumns(newColumns); setColumnOrder(newOrder)
    await updateDoc(doc(db, 'boards', boardId), { columns: newColumns, columnOrder: newOrder })
  ...
  async function renameColumn(colId, currentTitle) {
    ...
    const newColumns = columns.map(c => c.id === colId ? { ...c, title: title.trim() } : c)
  ...
  async function deleteColumn(colId) {
    ...
    if (colCards.length > 0) { if (!confirm(`"${col.title}" has ${colCards.length} card(s). Delete anyway?`)) return }
    else { if (!confirm(`Delete "${col.title}" column?`)) return }
  ...
  function handleDragStart(event) {
    // ── Block Firestore listener from updating state ──
    isDraggingRef.current = true
    pendingCardsRef.current = null

    const { type } = event.active.data.current
    if (type === 'column') { setActiveColumn(event.active.data.current.column); return }
  ...
  async function handleDragEnd(event) {
    ...
    if (!over) {
      // Unblock listener and apply any pending updates
      isDraggingRef.current = false
      if (pendingCardsRef.current) {
        setCards(prev => ({ ...prev, ...pendingCardsRef.current }))
        pendingCardsRef.current = null
      }
      return
    }
    ...
    if (!toCol || !columns.find(c => c.id === toCol)) {
      isDraggingRef.current = false
      return
    }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {board?.background && <div style={{ position: 'fixed', inset: 0, background: board.background, opacity: 0.06, pointerEvents: 'none', zIndex: 0 }}/>}
      ...
      <div style={{ background: 'rgba(13,13,13,0.9)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--border)', padding: '0 2rem', height: 56, display: 'flex', alignItems: 'center', gap: 16, position: 'sticky', top: 0, zIndex: 100, flexShrink: 0 }}>
        ...
        <div style={{ position: 'relative', flex: 1, maxWidth: 280 }}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text3)', pointerEvents: 'none' }}><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search cards..." style={{ width: '100%', padding: '7px 12px 7px 32px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, color: 'var(--text)', outline: 'none', fontFamily: 'Inter, sans-serif', transition: 'border-color 0.15s' }} onFocus={e => e.target.style.borderColor = 'var(--accent)'} onBlur={e => e.target.style.borderColor = 'var(--border)'} />
          {search && <button onClick={() => setSearch('')} style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 14, padding: 0 }}>×</button>}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          {board?.members?.slice(0, 5).map((uid, i) => (
            <div key={uid} style={{ width: 28, height: 28, borderRadius: '50%', background: `hsl(${(i * 60) % 360}, 70%, 60%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: 'white', border: '2px solid var(--bg)', marginLeft: i > 0 ? -8 : 0, zIndex: 5 - i, position: 'relative' }} title={uid === user.uid ? `You (${user.displayName || user.email})` : (memberProfiles[uid]?.name || memberProfiles[uid]?.email || 'Member')}>
              {uid === user.uid ? user.displayName?.[0]?.toUpperCase() : (memberProfiles[uid]?.name?.[0]?.toUpperCase() || '?')}
            </div>
          ))}
          {board?.members?.length > 5 && <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'var(--text2)', border: '2px solid var(--bg)', marginLeft: -8 }}>+{board.members.length - 5}</div>}
        </div>
        <button onClick={() => setShowAI(true)} style={{ padding: '6px 14px', background: 'linear-gradient(135deg, var(--accent), #a78bfa)', border: 'none', borderRadius: 8, color: 'white', fontSize: 12, fontWeight: 600, cursor: 'pointer', boxShadow: '0 4px 16px rgba(124,106,255,0.3)', transition: 'all 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-1px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>✨ AI Generate</button>
        <button onClick={() => setShowInvite(true)} style={{ padding: '6px 14px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border2)'; e.currentTarget.style.color = 'var(--text)' }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text2)' }}>👥 Invite</button>
        <button onClick={() => setShowShortcuts(prev => !prev)} style={{ padding: '6px 14px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>⌨️ Shortcuts</button>
        <button onClick={() => setShowStats(prev => !prev)} style={{ padding: '6px 14px', background: showStats ? 'var(--accent-dim)' : 'var(--surface2)', border: `1px solid ${showStats ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 8, color: showStats ? 'var(--accent)' : 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>📊 Stats</button>
        <button onClick={() => setShowActivity(prev => !prev)} style={{ padding: '6px 14px', background: showActivity ? 'var(--accent-dim)' : 'var(--surface2)', border: `1px solid ${showActivity ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 8, color: showActivity ? 'var(--accent)' : 'var(--text2)', fontSize: 12, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>📋 Activity</button>
      </div>
        ...
        <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
          <SortableContext items={columnOrder} strategy={horizontalListSortingStrategy}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', minWidth: 'max-content' }}>
              {columnOrder.map(colId => {
                ...
                return (
                  <KanbanColumn
                    key={col.id}
                    column={col}
                    cards={search.trim() ? (cards[col.id] || []).filter(c => c.title.toLowerCase().includes(search.toLowerCase()) || c.labels?.some(l => l.toLowerCase().includes(search.toLowerCase()))) : (cards[col.id] || [])}
                    onAddCard={addCard}
              ...
              <button onClick={addColumn} style={{ width: 300, flexShrink: 0, height: 54, background: 'rgba(255,255,255,0.03)', border: '2px dashed var(--border)', borderRadius: 16, color: 'var(--text3)', fontSize: 14, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s', fontFamily: 'Inter, sans-serif' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--accent)'; e.currentTarget.style.color = 'var(--accent)'; e.currentTarget.style.background = 'var(--accent-dim)' }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text3)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}>+ Add Column</button>
            </div>
          </SortableContext>
          <DragOverlay>
            {activeCard && (
            ...
            {activeColumn && (
              <div style={{ width: 300, background: 'var(--surface)', border: '1px solid var(--accent)', borderRadius: 16, padding: '14px 16px', boxShadow: '0 16px 48px rgba(0,0,0,0.6)', cursor: 'grabbing', transform: 'rotate(1deg)', opacity: 0.9 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: activeColumn.color }}/>
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>{activeColumn.title}</span>
                </div>
              </div>
            )}
          </DragOverlay>
      ...
      {showCardModal && <CardModal card={editingCard} onClose={() => { setShowCardModal(false); setEditingCard(null) }} onSave={saveCard} />}
      {openCard && <CardDetailModal card={openCard} columnId={openCardColumnId} boardId={boardId} onClose={() => { setOpenCard(null); setOpenCardColumnId(null) }} onSave={saveCard} onDelete={deleteCard} />}
      {showActivity && <ActivityPanel log={activityLog} onClose={() => setShowActivity(false)} />}
      {showInvite && <InviteModal boardId={boardId} board={board} user={user} onClose={() => setShowInvite(false)} memberProfiles={memberProfiles} />}
      {showShortcuts && <ShortcutsModal onClose={() => setShowShortcuts(false)} />}
      {showAI && <AIGenerateModal boardId={boardId} user={user} onClose={() => setShowAI(false)} onGenerate={generateTasks} />}
      {showStats && <StatsPanel cards={cards} columns={columns} columnOrder={columnOrder} board={board} onClose={() => setShowStats(false)} />}

      {toastMsg && (
        <div style={{ position: 'fixed', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', background: 'var(--accent)', color: 'white', padding: '12px 24px', borderRadius: 30, fontSize: 14, fontWeight: 600, boxShadow: '0 8px 32px rgba(124,106,255,0.4)', zIndex: 9999, animation: 'slideUp 0.3s ease' }}>
          ✨ {toastMsg}
        </div>
      )}
    </div>
